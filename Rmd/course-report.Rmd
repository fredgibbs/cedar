...
```{r setup, fig.width = 10, fig.height = 12, echo = FALSE,warning = FALSE, message = FALSE}
library(kableExtra)
library(rmarkdown)
library(dplyr)
library(tidyr)
...
cat(paste0("#### ","More EARLY DROPS than usual\n"))
...
cat(paste0("#### ","More LATE DROPS than usual\n"))

drops <- late_drops %>% arrange(term_type,`Academic Period Code`) 
...
forecast_filtered <- forecast_course %>% select(cols) %>% arrange(CAMP,TERM,SUBJ_CRSE)
