...
reset_forecast_course <- function(opt) {
  ...
  forecast_rds_file <- paste0(cedar_data_dir,"forecasts.Rds")
...
add_to_forecast_table <- function(new_forecast_row) {
  ...
  # save new data
  # if running in posit connect, save to root directory; otherwise save to cedar_data_dir
  if (as.logical(Sys.getenv("shiny")) && !as.logical(Sys.getenv("docker"))) {
