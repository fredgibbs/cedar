...
# this func is run from either cedar.R, CLI, or from cedar() if running interactively
# no data loading should occur here
...
process_func <- function(opt) {
  ...
  if (opt$func == "credit-hours") {
    message("[process_func.R] CREDIT HOURS!")
    if (opt$guide == TRUE) {
      message ("
    ...
    process_output(credit_hours_data,"credit_hours",opt) #.csv is added in process_output
    
    return("[process+func.R] done processing credit-hours!")
  }
  ...
  if (opt$func == "regstats") {
    ...
    # check if wanting shiny dashboard data
      ...
      message("[process_func.R] Generating regstats data for Shiny dashboard...")
      ...
      message("[process_func.R] Regstats dashboard data saved to: ", output_file)
      message("[process_func.R] Total flagged courses: ", length(regstat_out$all_flagged_courses))
